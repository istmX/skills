...
Read `.istm-context/design.md` in full: character, the build mandate, composition patterns, and every rule in `## Component & usage rules` (do's and don'ts). Then read the real token values from the CSS token file (`globals.css` / tailwind config) it points to: colors (light + dark), typography, spacing, radius, shadows, motion. The CSS is the source of truth for values; `.istm-context/design.md` is the source of truth for direction. (If an older `.istm-context/design.md` still carries inline token values, treat the CSS as authoritative where they disagree, and note it in the report.)

### DS2: Confirm token coverage

Run stack, styling, and dark mode detection. If the page needs a token the CSS does not have yet, add it in the system's language (derive from the nearest existing token, follow the `.istm-context/design.md` rules), never a one off hardcoded value. Never silently overwrite an existing token; a genuine conflict → list it in the report, let the engineer decide.

### DS3: Implement

Run the implementation phases (`ui/implementation.md`): Phase 0 composes the full surface in the existing system, Phases 1 to 5 integrate it, Phase 6 audits the build against the bar. `.istm-context/design.md` governs direction, the CSS tokens govern values.
