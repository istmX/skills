...
**Prove it before you report.** Before declaring the UI done, audit the build yourself against these disqualifiers and against the page's `.istm-context/design.md` mandate, and fix every hit. When you have a browser or screenshot tool, render the page and look at it, the way a designer checks their own work, and fix any visual defect you see (the only reliable way to catch a broken render like a stray black bar). Say in the report what you audited.
...
- A described direction, a brand or site URL, or nothing at all → `ui/generate.md` (B1 finds the direction, B2 derives and verifies the tokens into CSS and the character into `.istm-context/design.md`), then implement.

No spec governs this UI, or the spec is silent on the design system → fall through to the detection in Steps 0.1 to 0.3. If detection settles nothing, ask *"How should I get the design for this?"* with options **From Figma (its MCP)** · **From a screenshot / images** · **From the existing `.istm-context/design.md` / current UI** · **No design, suggest a direction** (the picker adds Other), then proceed by their pick.

## Step 0.1: Check for existing .istm-context/design.md

File search for `.istm-context/design.md` within about 3 levels of the project root, ignoring `node_modules`, `.git`, `.claude`. Found: validate; a real one has a `character` and a `## Build mandate` (art direction) and points at the CSS for tokens. Empty, or only a stub: treat as not found and warn the user. (An older `.istm-context/design.md` that still carries inline `colors:` / `typography:` value blocks is still valid; treat the CSS as authoritative for values.)
...
## Step 0.2: Brownfield check (no .istm-context/design.md, but is there existing UI?)
...
- **question**: "There's no `.istm-context/design.md`, but this project already has UI. How should I get the design system?"
- **header**: "Design system"
- **options**:
  1. `Extract from existing code`: "Recommended, capture the design direction from the current tokens/components into `.istm-context/design.md`, pointing at the existing CSS tokens, so new UI matches what's shipped." → **Step 0.3**.
  2. `Use a reference`: "I'll give a screenshot, a `.istm-context/design.md` URL, or a site to work from." → **Step 1**.
...
- `ui/existing.md` when an existing `.istm-context/design.md` or current UI is the source.
...
- Project design system: `./.istm-context/design.md` (art direction and the build mandate; token values live in CSS)
