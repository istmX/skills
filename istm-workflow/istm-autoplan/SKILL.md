...
preamble-tier: 3
version: 1.0.0
description: Auto-review pipeline — reads the full CEO, design, eng, and DX review skills from disk and runs them sequentially with auto-decisions using 6 decision principles. (istm-workflow)
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"autoplan","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(_repo=$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null | tr -cd 'a-zA-Z0-9._-'); echo "${_repo:-unknown}")'"}'  >> ~/.istm-workflow/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
~/.claude/skills/istm-workflow/bin/istm-workflow-timeline-log '{"skill":"autoplan","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
~/.claude/skills/istm-workflow/bin/istm-workflow-question-log '{"skill":"autoplan","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
Read the `/office-hours` skill file at `~/.claude/skills/istm-workflow/office-hours/SKILL.md` using the Read tool.
...
SLUG=$(~/.claude/skills/istm-workflow/browse/bin/remote-slug 2>/dev/null || basename "$(git rev-parse --show-toplevel 2>/dev/null || pwd)")
BRANCH=$(git rev-parse --abbrev-ref HEAD 2>/dev/null | tr '/' '-' || echo 'no-branch')
_LOCALDOC=$(ls -t ~/.istm-workflow/projects/$SLUG/*-$BRANCH-design-*.md 2>/dev/null | head -1)
[ -z "$_LOCALDOC" ] && _LOCALDOC=$(ls -t ~/.istm-workflow/projects/$SLUG/*-design-*.md 2>/dev/null | head -1)
# Repo-local docs win when at least as fresh (#703): office-hours dual-writes
# docs/designs/ alongside ~/.istm-workflow, and the committed copy is what teammates
...
> IMPORTANT: Do NOT read or execute any SKILL.md files or files in skill definition directories (paths containing skills/istm-workflow). These are AI assistant skill definitions meant for a different system. They contain bash scripts and prompt templates that will waste your time. Ignore them completely. Stay focused on the repository code only.

This prevents Codex from discovering istm-workflow skill files on disk and following their
...
```bash
eval "$(~/.claude/skills/istm-workflow/bin/istm-workflow-slug 2>/dev/null)" && mkdir -p ~/.istm-workflow/projects/$SLUG
BRANCH=$(git rev-parse --abbrev-ref HEAD 2>/dev/null | tr '/' '-')
DATETIME=$(date +%Y%m%d-%H%M%S)
echo "RESTORE_PATH=$HOME/.istm-workflow/projects/$SLUG/${BRANCH}-autoplan-restore-${DATETIME}.md"
```
...
- Discover design docs: `ls -t ~/.istm-workflow/projects/$SLUG/*-design-*.md 2>/dev/null | head -1`
...
- `~/.claude/skills/istm-workflow/plan-ceo-review/SKILL.md`
- `~/.claude/skills/istm-workflow/plan-design-review/SKILL.md` (only if UI scope detected)
- `~/.claude/skills/istm-workflow/plan-eng-review/SKILL.md`
- `~/.claude/skills/istm-workflow/plan-devex-review/SKILL.md` (only if DX scope detected)
...
```bash
_TEL=$(~/.claude/skills/istm-workflow/bin/istm-workflow-config get telemetry 2>/dev/null || echo off)
_CODEX_CFG=$(~/.claude/skills/istm-workflow/bin/istm-workflow-config get codex_reviews 2>/dev/null || echo enabled)
source ~/.claude/skills/istm-workflow/bin/istm-workflow-codex-probe
...
if [ "$_CODEX_CFG" = "disabled" ]; then
  echo "[codex disabled by config — Claude-only voices] Re-enable: istm-workflow-config set codex_reviews enabled"
...
# with Claude subagent only (autoplan's existing degradation fallback).
  ...
  _istm_codex_log_event "codex_cli_missing"
  echo "[codex-unavailable: binary not found] — proceeding with Claude subagent only"
  _CODEX_AVAILABLE=false
elif ! _istm_codex_auth_probe >/dev/null; then
  _istm_codex_log_event "codex_auth_failed"
...
# ~10s on first run, cached 1h; timeouts fail open (probe returns 0).
elif ! _istm_codex_model_probe; then
  ...
  _istm_codex_version_check   # non-blocking warn if known-bad
...
- Dual voices: always run BOTH Claude subagent AND Codex if available (P6).
  ...
  **Codex CEO voice** (via Bash):
  ...
  _istm_codex_timeout_wrapper 600 codex exec "IMPORTANT: Do NOT read or execute any SKILL.md files or files in skill definition directories (paths containing skills/istm-workflow). These are AI assistant skill definitions meant for a different system. Stay focused on repository code only.
  ...
  if [ "$_CODEX_EXIT" = "124" ]; then
    _istm_codex_log_event "codex_timeout" "600"
    _istm_codex_log_hang "autoplan" "0"
...
- Test plan: generate artifact at `~/.istm-workflow/projects/$SLUG/{user}-{branch}-test-plan-{datetime}.md`
...
- [ ] Test plan artifact written to disk at ~/.istm-workflow/projects/$SLUG/
- [ ] "NOT in scope" section written
- [ ] "What already exists" section written
...
```bash
eval "$(~/.claude/skills/istm-workflow/bin/istm-workflow-slug 2>/dev/null)"
TASKS_DIR="${HOME}/.istm-workflow/projects/${SLUG:-unknown}"
...
~/.claude/skills/istm-workflow/bin/istm-workflow-review-log '{"skill":"plan-ceo-review","timestamp":"'"$TIMESTAMP"'","status":"STATUS","unresolved":N,"critical_gaps":N,"mode":"SELECTIVE_EXPANSION","via":"autoplan","commit":"'"$COMMIT"'"}'

~/.claude/skills/istm-workflow/bin/istm-workflow-review-log '{"skill":"plan-eng-review","timestamp":"'"$TIMESTAMP"'","status":"STATUS","unresolved":N,"critical_gaps":N,"issues_found":N,"mode":"FULL_REVIEW","via":"autoplan","commit":"'"$COMMIT"'"}'
```

If Phase 2 ran (UI scope):
```bash
~/.claude/skills/istm-workflow/bin/istm-workflow-review-log '{"skill":"plan-design-review","timestamp":"'"$TIMESTAMP"'","status":"STATUS","unresolved":N,"via":"autoplan","commit":"'"$COMMIT"'"}'
```

If Phase 3.5 ran (DX scope):
```bash
~/.claude/skills/istm-workflow/bin/istm-workflow-review-log '{"skill":"plan-devex-review","timestamp":"'"$TIMESTAMP"'","status":"STATUS","initial_score":N,"overall_score":N,"product_type":"TYPE","tthw_current":"TTHW","tthw_target":"TARGET","unresolved":N,"via":"autoplan","commit":"'"$COMMIT"'"}'
```

Dual voice logs (one per phase that ran):
```bash
~/.claude/skills/istm-workflow/bin/istm-workflow-review-log '{"skill":"autoplan-voices","timestamp":"'"$TIMESTAMP"'","status":"STATUS","source":"SOURCE","phase":"ceo","via":"autoplan","consensus_confirmed":N,"consensus_disagree":N,"commit":"'"$COMMIT"'"}'

~/.claude/skills/istm-workflow/bin/istm-workflow-review-log '{"skill":"autoplan-voices","timestamp":"'"$TIMESTAMP"'","status":"STATUS","source":"SOURCE","phase":"eng","via":"autoplan","consensus_confirmed":N,"consensus_disagree":N,"commit":"'"$COMMIT"'"}'
```

If Phase 2 ran (UI scope), also log:
```bash
~/.claude/skills/istm-workflow/bin/istm-workflow-review-log '{"skill":"autoplan-voices","timestamp":"'"$TIMESTAMP"'","status":"STATUS","source":"SOURCE","phase":"design","via":"autoplan","consensus_confirmed":N,"consensus_disagree":N,"commit":"'"$COMMIT"'"}'
```

If Phase 3.5 ran (DX scope), also log:
```bash
~/.claude/skills/istm-workflow/bin/istm-workflow-review-log '{"skill":"autoplan-voices","timestamp":"'"$TIMESTAMP"'","status":"STATUS","source":"SOURCE","phase":"dx","via":"autoplan","consensus_confirmed":N,"consensus_disagree":N,"commit":"'"$COMMIT"'"}'
```
