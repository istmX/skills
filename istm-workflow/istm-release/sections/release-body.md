...
(adapt to whatever project you're in — these are not istm-workflow-specific):
...
**If GitHub:**
```bash
gh pr view --json body -q .body > /tmp/istm-workflow-pr-body-$$.md
cp /tmp/istm-workflow-pr-body-$$.md /tmp/istm-workflow-pr-body-orig-$$.md
```

**If GitLab:**
```bash
glab mr view -F json 2>/dev/null | python3 -c "import sys,json; print(json.load(sys.stdin).get('description',''))" > /tmp/istm-workflow-pr-body-$$.md
cp /tmp/istm-workflow-pr-body-$$.md /tmp/istm-workflow-pr-body-orig-$$.md
```
...
```bash
~/.claude/skills/istm-workflow/bin/istm-workflow-issue-guard --stdin --source pr-body < /tmp/istm-workflow-pr-body-$$.md
```
...
4. Redaction scan-at-sink, then write the updated body back. The body is already
   in a temp file (`/tmp/istm-workflow-pr-body-$$.md`); scan THAT file before editing so
   the bytes scanned are the bytes sent:

```bash
REDACT_VIS=$(~/.claude/skills/istm-workflow/bin/istm-workflow-config get redact_repo_visibility 2>/dev/null)
[ -z "$REDACT_VIS" ] && REDACT_VIS=$(gh repo view --json visibility -q .visibility 2>/dev/null | tr 'A-Z' 'a-z')
~/.claude/skills/istm-workflow/bin/istm-workflow-redact --from-file /tmp/istm-workflow-pr-body-$$.md --repo-visibility "${REDACT_VIS:-unknown}" --json
...
if [ ! -f /tmp/istm-workflow-pr-body-orig-$$.md ] || [ ! -f /tmp/istm-workflow-pr-body-$$.md ]; then
...
_ORIG_BANNERS=$(grep -c "UNTRUSTED TRACKER CONTENT" /tmp/istm-workflow-pr-body-orig-$$.md 2>/dev/null)
_ORIG_BANNERS=${_ORIG_BANNERS:-0}
_NEW_BANNERS=$(grep -c "UNTRUSTED TRACKER CONTENT" /tmp/istm-workflow-pr-body-$$.md 2>/dev/null)
...
**If GitHub:**
```bash
gh pr edit --body-file /tmp/istm-workflow-pr-body-$$.md
```

**If GitLab:**
Read the contents of `/tmp/istm-workflow-pr-body-$$.md` using the Read tool, then pass it to `glab mr update` using a heredoc to avoid shell metacharacter issues:
```bash
...
```bash
rm -f /tmp/istm-workflow-pr-body-$$.md /tmp/istm-workflow-pr-body-orig-$$.md
```
...
```bash
NEW_TITLE=$(~/.claude/skills/istm-workflow/bin/istm-workflow-pr-title-rewrite.sh "$V" "$CURRENT_TITLE")
```
...
stays discoverable: "Running the Codex doc review automatically (standard step). Disable: `istm-workflow-config set codex_reviews disabled`."
...
**Persist the result:**
```bash
~/.claude/skills/istm-workflow/bin/istm-workflow-review-log '{"skill":"codex-doc-review","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","commit":"'"$(git rev-parse --short HEAD)"'"}'
```
