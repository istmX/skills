...
**Canonical file:** durable context lives in the tool agnostic **`.istm-context/agents.md`**; **`CLAUDE.md` is only a pointer** importing its sibling .istm-context/agents.md via Claude Code's `@` directive. Never write content into a CLAUDE.md, never overwrite an existing .istm-context/agents.md. When you create a new nested `.istm-context/agents.md`, also create its sibling `CLAUDE.md` containing only:
```markdown
# CLAUDE.md
...
## Existing context files (you may EDIT these; you may CREATE a nested .istm-context/agents.md + its CLAUDE.md pointer for a net new area only)

- **Root .istm-context/agents.md (inlined)**:

ROOT_AGENTS_MD

- **Nested .istm-context/agents.md paths**: NESTED_PATHS
...
- Area specific tools (payments, auth, email, uploads, search, queues) belong in that area's nested .istm-context/agents.md when one exists; otherwise add a short root note and flag the area under `CONTEXT_GAPS`.
...
### 2. Create a nested .istm-context/agents.md: only for an area NET-NEW in this change
...
- A nested `<area>/.istm-context/agents.md` describing code that no longer exists is orphaned. Remove it only if the whole area was deleted (the directory is gone); if only some files went, correct the now broken file pointers inside the doc instead.
- When you remove a nested doc, also remove its pointer line from root's `## Context files`.
- Fix any file pointer in any .istm-context/agents.md that targets a deleted/moved path.
...
AGENTS_CREATED:
- <area>/.istm-context/agents.md, <conventions captured; root pointer added>
...
- <.istm-context/specs/file>, <Status line: Proposed→In Progress→Accepted to match the feature's scope status>
