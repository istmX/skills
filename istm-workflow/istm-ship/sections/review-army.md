...
1. Read `~/.claude/skills/istm-workflow/review/checklist.md`. If the file cannot be read, **STOP** and report the error.
...
Check if the diff touches frontend files using `istm-workflow-diff-scope`:

```bash
source <(~/.claude/skills/istm-workflow/bin/istm-workflow-diff-scope <base> 2>/dev/null)
```
...
```bash
~/.claude/skills/istm-workflow/bin/istm-workflow-review-log '{"skill":"design-review-lite","timestamp":"TIMESTAMP","status":"STATUS","findings":N,"auto_fixed":M,"commit":"COMMIT"}'
```
...
```bash
source <(~/.claude/skills/istm-workflow/bin/istm-workflow-diff-scope <base> 2>/dev/null) || true
...
```bash
~/.claude/skills/istm-workflow/bin/istm-workflow-specialist-stats 2>/dev/null || true
```
...
1. **Testing** — read `~/.claude/skills/istm-workflow/review/specialists/testing.md`
2. **Maintainability** — read `~/.claude/skills/istm-workflow/review/specialists/maintainability.md`
...
3. **Security** — if SCOPE_AUTH=true, OR if SCOPE_BACKEND=true AND DIFF_LINES > 100. Read `~/.claude/skills/istm-workflow/review/specialists/security.md`
4. **Performance** — if SCOPE_BACKEND=true OR SCOPE_FRONTEND=true. Read `~/.claude/skills/istm-workflow/review/specialists/performance.md`
5. **Data Migration** — if SCOPE_MIGRATIONS=true. Read `~/.claude/skills/istm-workflow/review/specialists/data-migration.md`
6. **API Contract** — if SCOPE_API=true. Read `~/.claude/skills/istm-workflow/review/specialists/api-contract.md`
7. **Design** — if SCOPE_FRONTEND=true. Use the existing design review checklist at `~/.claude/skills/istm-workflow/review/design-checklist.md`

### Adaptive gating

After scope-based selection, apply adaptive gating based on specialist hit rates:

For each conditional specialist that passed scope gating, check the `istm-workflow-specialist-stats` output above:
...
```bash
~/.claude/skills/istm-workflow/bin/istm-workflow-learnings-search --type pitfall --query "{specialist domain}" --limit 5 2>/dev/null || true
```
...
1. The red-team checklist from `~/.claude/skills/istm-workflow/review/specialists/red-team.md`
...
```bash
~/.claude/skills/istm-workflow/bin/istm-workflow-review-log '{"skill":"review","timestamp":"TIMESTAMP","status":"STATUS","issues_found":N,"critical":N,"informational":N,"quality_score":SCORE,"specialists":SPECIALISTS_JSON,"findings":FINDINGS_JSON,"commit":"'"$(git rev-parse --short HEAD)"'","via":"ship"}'
```
