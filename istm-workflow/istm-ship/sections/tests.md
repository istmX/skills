...
`~/.istm-workflow/projects/<slug>/<branch>-evidence.jsonl` — Step 16 cites this
record instead of re-running when the content hasn't changed:

```bash
~/.claude/skills/istm-workflow/bin/istm-workflow-evidence run --label tests -- 'bin/test-lane 2>&1' &
~/.claude/skills/istm-workflow/bin/istm-workflow-evidence run --label vitest -- 'npm run test 2>&1' &
wait
```

After both complete, check the `istm-workflow-evidence: recorded label=... exit=...
...
- Create an issue assigned to that person (use the platform detected in Step 0):
  - **If GitHub:**
    ...
    gh issue create \
      ...
      --body "Found failing on branch <current-branch>. Failure is pre-existing.\n\n**Error:**\n```\n<first 10 lines>\n```\n\n**Last modified by:** <author>\n**Noticed by:** istm-workflow /ship on <date>" \
...
`~/.claude/skills/istm-workflow/bin/istm-workflow-detach` instead — it survives in its own
session, serializes against other worktrees via a machine lock (no API
saturation), and writes a guaranteed `### istm-workflow-detach EXIT=<code> ###` sentinel:

```bash
~/.claude/skills/istm-workflow/bin/istm-workflow-detach --label ship-evals --lock istm-workflow-evals --timeout 5400 -- <project eval command>
```
